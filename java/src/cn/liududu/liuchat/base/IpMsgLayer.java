    ...
    public void dispatch(IpMsg ipMsg) {
        switch (ipMsg.getCommand()) {
            ...
            case IpMsgCommand.IPMSG_GETFILEDATA:
                ...
                break;
            //todo  拒绝文件传输的处理
            case IpMsgCommand.IPMSG_DECLINEFILEDATA:
                Logger.log("收到文件传输的拒绝请求");
                md.handleDeclineFileTransferRequest(ipMsg);
                break;
            case IpMsgCommand.IPMSG_ACCEPTFILEDATA:
                Logger.log("收到文件传输的同意请求 ");
                md.handleAcceptFileTransferRequest(ipMsg);
                break;
