...
public class MsgDispatcher {
    ...
    * */
    public void handleFileTransferRequest(IpMsg ipMsg) {
         ...
         if (result == JOptionPane.YES_OPTION) {
             ...
             new FileTransferRequest().AcceptRequest(ipMsg.getAddr().toString().split("/")[1],fileInfo);
         } else{
             //拒绝文件传输请求
    ...
    * */
    public void handleDeclineFileTransferRequest(IpMsg ipMsg) {
        JOptionPane.showMessageDialog(MainWindow.getInstance(),"对方拒绝了文件传输请求");
        SendFileServer.close();
    }
    ...
    * */
    public void handleAcceptFileTransferRequest(IpMsg ipMsg) {
        JOptionPane.showMessageDialog(MainWindow.getInstance(),"对方同意了文件传输请求");
    }
}
